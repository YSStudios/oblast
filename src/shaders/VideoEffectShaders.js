...
export const videoEffectFragmentShader = `
  ...
  uniform float uVideoWidthScale;
  ...
  void main() {
    ...
    // Rotate UV coordinates 90 degrees counter-clockwise with correct orientation
    vec2 rotatedUV = vec2(1.0 - uv.y, 1.0 - uv.x);
    
    // Simple scaling approach - scale UV coordinates to make video smaller
    float videoScale = 3.0; // Higher number = smaller video
    vec2 videoScaleVec = vec2(videoScale * uVideoWidthScale, videoScale);
    vec2 scaledUV = (rotatedUV - 0.5) * videoScaleVec + 0.5;
    
    // Adjust position to center the video properly on the mesh
    vec2 offset = vec2(-0.48, -0.55);
    vec2 mappedUV = scaledUV + offset;
    
    // Use the mapped UV for all calculations
    uv = mappedUV;
